...
namespace hpx { namespace util { namespace detail {
    ...
    void sprintf_formatter::_format(std::ostream& os, std::vector<char>& buffer,
        char const* format_str, ...)
    {
        std::va_list arg1;
        va_start(arg1, format_str);
        std::va_list arg2;
        va_copy(arg2, arg1);
        ...
        length = std::vsnprintf(buffer.data(), buffer.size(), format_str, arg1);
        va_end(arg1);
        if (length > buffer.size())
        {
            buffer.resize(length);
            length =
                std::vsnprintf(buffer.data(), buffer.size(), format_str, arg2);
        }
        va_end(arg2);
        ...
        std::ostream& os, char const* value, std::size_t len) const
    {
        ...
        return sprintf_formatter::_format(os, _buffer, _format_str, value);
    }
